...
class KoshaAPIClient:
    ...
    def get_full_substance_info(self, cas_no: str, full=True):
        ...
        legal_names = [] # [V18.0.0.0] 괄호 명칭 저장소
        if osh_text:
            osh_info = {
            ...
            # [V18.0.0.0] 작업환경측정대상물질 (괄호안 명칭) 추출 - 금홍석 사례 대응
            match = re.search(r'작업환경측정대상물질\s*\(([^)]+)\)', osh_text)
            if match:
                legal_names = [n.strip() for n in match.group(1).split(',')]
        ...
        return {
            ...
            "osh": osh_info,
            "cca": cca_info,
            "legal_names": legal_names # [V18.0.0.0] 추가된 법적 명칭 리스트
        }
