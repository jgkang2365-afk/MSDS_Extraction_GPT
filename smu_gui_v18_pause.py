...
from PyQt5.QtWidgets import (
    ...
    QStackedWidget, QToolButton, QSizePolicy, QMenu, QDialog, QCheckBox
)

# [V18.0.0.0] MES 매핑 정밀 검토 다이얼로그 (Lazy Review)
class ReviewDialog(QDialog):
    def __init__(self, review_items, parent=None):
        super().__init__(parent)
        self.setWindowTitle("MES 매핑 정밀 검토 (Lazy Review) - v3.1")
        self.setMinimumSize(1000, 600)
        self.review_items = review_items # [{"filename", "cas", "original_name", "mapped_name", "row_idx"}]
        self.checkboxes = [] # 각 행별 체크박스 리스트 보관
        self.init_ui()

    def init_ui(self):
        layout = QVBoxLayout(self)
        
        header = QLabel("🔍 AI 매핑이 모호하거나 다중 매핑된 항목들입니다. [적합한 국내 규정 명칭]을 선택한 후 승인해주세요.")
        header.setStyleSheet("font-weight: bold; color: #0056b3; margin-bottom: 10px;")
        layout.addWidget(header)

        self.table = QTableWidget()
        self.table.setColumnCount(5)
        self.table.setHorizontalHeaderLabels(["파일명", "CAS 번호", "법적 명칭 (Sec.15)", "MES 매핑 후보 (판단 후 선택)", "행 승인"])
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Interactive)
        self.table.horizontalHeader().setSectionResizeMode(3, QHeaderView.Stretch) # 후보 열은 넓게
        self.table.setRowCount(len(self.review_items))

        for i, item in enumerate(self.review_items):
            self.table.setItem(i, 0, QTableWidgetItem(item.get("filename", "")))
            self.table.setItem(i, 1, QTableWidgetItem(item.get("cas", "")))
            self.table.setItem(i, 2, QTableWidgetItem(item.get("original_name", "")))
            
            # [V3.1] 매핑된 후보들을 체크박스 그룹으로 생성
            mapped_str = item.get("mapped_name", "")
            candidates = [c.strip() for c in mapped_str.split(";") if c.strip()]
            
            cand_widget = QWidget()
            cand_layout = QVBoxLayout(cand_widget)
            cand_layout.setContentsMargins(5, 5, 5, 5)
            cand_layout.setSpacing(2)
            
            row_cbs = []
            for cand in candidates:
                cb = QCheckBox(cand)
                cb.setChecked(True) # 기본적으로 모두 선택
                cand_layout.addWidget(cb)
                row_cbs.append(cb)
            
            self.checkboxes.append(row_cbs)
            self.table.setCellWidget(i, 3, cand_widget)
            # 행 높이를 후보 개수에 맞춰 자동 조절
            self.table.setRowHeight(i, max(60, len(candidates) * 25 + 15))

            # 행 전체 승인 여부
            row_cb_container = QWidget()
            row_cb_layout = QHBoxLayout(row_cb_container)
            row_cb = QCheckBox()
            row_cb.setChecked(True)
            row_cb_layout.addWidget(row_cb)
            row_cb_layout.setAlignment(Qt.AlignCenter)
            row_cb_layout.setContentsMargins(0, 0, 0, 0)
            self.table.setCellWidget(i, 4, row_cb_container)

        layout.addWidget(self.table)

        btn_layout = QHBoxLayout()
        self.btn_confirm = QPushButton("✅ 선택 항목 일괄 승인 및 반영")
    ...
    def get_results(self):
        """[V3.1] 주님이 선택하신 명칭들만 취합하여 반환"""
        results = []
        for i in range(self.table.rowCount()):
            # 행 승인 여부 확인
            row_cb_container = self.table.cellWidget(i, 4)
            row_cb = row_cb_container.findChild(QCheckBox)
            if row_cb and row_cb.isChecked():
                item = self.review_items[i].copy()
                
                # 체크된 명칭들만 수집
                selected_names = []
                for cb in self.checkboxes[i]:
                    if cb.isChecked():
                        selected_names.append(cb.text())
                
                # [V3.5] 정밀 교체용 후보군 정보 포함
                # item["candidates"]는 이미 on_validation_result에서 넣어둠
                item["mapped_name"] = "; ".join(selected_names)
                results.append(item)
        return results
    ...
    def run(self):
        ...
        for i, data in enumerate(self.table_data):
            ...
            try:
                ...
                review_list = [] # [V18.0.0.0] 후검토 목록 초기화
                ...
                for c in raw_val_res.get("components", []):
                    ...
                    # [V18.0.0.0] 코어 정밀 매핑 결과 우선 사용
                    clean_name = name.strip()
                    # [V3.7] 마스터 DB TWA 노이즈 제거 (괄호 안의 ppm, mg/m3 형식 포함)
                    # 광물성분진 등의 내용은 살리고, 순수 노출기준 수치 형식만 제거
                    clean_name = re.sub(r'\s*\(\d+[\d\.]*\s*(?:ppm|mg/m3|mg/㎥)\)', '', clean_name, flags=re.IGNORECASE)
                    clean_name = re.sub(r'\s*\((?:STEL|TWA|PEL|TLV)\)', '', clean_name, flags=re.IGNORECASE).strip()

                    # [NEW] 후검토 데이터 수집
                    if c.get("review_needed"):
                        m_res = c.get('match_results', [])
                        cands_display = [m.get('display_name', m['name']) for m in m_res]
                        cands_clean = [m['name'] for m in m_res] # [V3.9] TWA 없는 순수 명칭들
                        
                        name_map = {m.get('display_name', m['name']): m['name'] for m in m_res}
                        
                        review_list.append({
                            "filename": fn, "cas": cas,
                            "original_name": c.get("legal_names")[0] if c.get("legal_names") else "미확인",
                            "mapped_name": "; ".join(cands_display), 
                            "name_map": name_map,
                            "candidates": list(set(cands_display + cands_clean)), # 중복 제거하여 합침
                            "content": range_val, 
                            "row_idx": row_idx
                        })
                    ...
                    # 2. 2차 결과: 규제 대상이면 함유량과 무관하게 기재 (주님 지침)
                    if is_work or is_spec:
                        res_2nd_list.append(f"{prefix}{clean_name}{c_str}")
                        
                    # 3. 측정대상 (Column 4): 오직 1% 이상만 기재 (비대상 문구 삭제)
                    if is_work and engine.is_measurement_target(range_val):
                        res_work_subjects.append(f"{clean_name}{c_str}")

                # [V3.8] Column 4용 최종 문자열 (1% 미만 완전 제외)
                final_work_str = "; ".join(res_work_subjects)
                ...
                res_data = {
                    ...
                    "validation": val_res,
                    "status": "검증 완료",
                    "review_list": review_list # [NEW] 후검토용 데이터 전달
                }
                self.result_signal.emit(res_data)
                self.progress_signal.emit(int((i + 1) / total * 100))
            except Exception as e:
...
class SMUGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.pending_reviews = [] # [V18.0.0.0] 후검토 대기 목록
        self.init_ui()
    ...
    def init_ui(self):
        self.setWindowTitle("1SMU MSDS Intelligence v3.0 - Enterprise Intelligence")
    ...
    def _create_settings_page(self):
        ...
        info_layout.addWidget(QLabel("MSDS Intelligence v3.0 - Enterprise Intelligence"))
    ...
    def run_validation(self):
        """[V18.0.0.0] KOSHA API 교차 검증 및 검토 목록 초기화"""
        self.pending_reviews = [] # [NEW] 새 작업 시작 시 비우기
    ...
    def on_validation_result(self, res_data):
        """[V18.0.0.0] 하이브리드 검증 결과 처리 및 후검토 목록 수집"""
        # [V18.0.0.0] 후검토 항목 누적 (모호한 매핑 발견 시)
        new_reviews = res_data.get("review_list", [])
        if new_reviews:
            self.pending_reviews.extend(new_reviews)
            self.log(f"[*] {res_data.get('filename', 'unknown')}: 모호한 매핑 {len(new_reviews)}건 검토 대기열 추가")
    ...
    def on_validation_finished(self):
        """[V18.0.0.0] 2단계 API 검증 완료 및 Lazy Review 창 실행"""
        self.table.blockSignals(False)
        self.btn_stop.setEnabled(False)
        self.btn_step1.setEnabled(True)
        self.btn_step2.setEnabled(True)
        ...
        # [V18.0.0.0] 모호한 매핑이 있는 경우 후검토 다이얼로그 실행
        if self.pending_reviews:
            self.log(f"[!] 총 {len(self.pending_reviews)}건의 모호한 매핑에 대한 사용자 확인이 필요합니다.")
            dialog = ReviewDialog(self.pending_reviews, self)
            if dialog.exec_():
                final_results = dialog.get_results()
                # [V18.7] 선택 결과를 행(Row)별로 정밀 교환 (Smart Sync & Filter)
                row_updates = {}
                for res in final_results:
                    row = res.get("row_idx")
                    if row not in row_updates: row_updates[row] = []
                    row_updates[row].append(res)
                
                for row, res_list in row_updates.items():
                    if row is not None and 0 <= row < self.table.rowCount():
                        # 1. 기존 UI 텍스트 가져오기 (Col 4: 측정대상, Col 5: 2차 결과)
                        old_measure_item = self.table.item(row, 4)
                        old_reg2_item = self.table.item(row, 5)
                        
                        measure_text = old_measure_item.text().strip() if old_measure_item else ""
                        reg2_text = old_reg2_item.text().strip() if old_reg2_item else ""
                        
                        current_measure_names = [n.strip() for n in measure_text.split(';') if n.strip()]
                        current_reg2_names = [n.strip() for n in reg2_text.split(';') if n.strip()]
                        
                        # 2. 정밀 교체 시작
                        for res in res_list:
                            candidates = res.get("candidates", [])
                            selected_display = res.get("mapped_name")
                            name_map = res.get("name_map", {})
                            content_val = res.get("content", "") # 원래 함유량
                            
                            clean_name = name_map.get(selected_display, selected_display)
                            # 주님이 원하시는 포맷: 명칭(함유량%)
                            formatted_name = f"{clean_name}({content_val})" if content_val else clean_name
                            
                            # 기존 텍스트에서 후보군 흔적 삭제 (함유량 패턴 포함)
                            def clean_list(name_list, cands):
                                new_list = []
                                for n in name_list:
                                    is_target_cand = False
                                    for cand in cands:
                                        # 후보 명칭이 포함되어 있으면 삭제 대상
                                        if cand in n: is_target_cand = True
                                    if not is_target_cand: new_list.append(n)
                                return new_list

                            current_measure_names = clean_list(current_measure_names, candidates)
                            current_reg2_names = clean_list(current_reg2_names, candidates)
                            
                            # [V3.8] 1% 필터링: 1% 이상일 때만 측정대상(Col 4)에 추가
                            is_over_1 = engine.is_measurement_target(content_val)
                            if is_over_1:
                                if formatted_name not in current_measure_names:
                                    current_measure_names.append(formatted_name)
                            
                            # 2차 결과(Col 5)에는 함유량 상관없이 추가
                            if formatted_name not in current_reg2_names:
                                current_reg2_names.append(formatted_name)
                        
                        final_measure_str = ";\n".join(current_measure_names)
                        final_reg2_str = ";\n".join(current_reg2_names)
                        
                        self.table.setItem(row, 4, QTableWidgetItem(final_measure_str))
                        ...
                        if hash_item:
                            ...
                            if f_hash in self.cache:
                                if "manual_data" not in self.cache[f_hash]:
                                    self.cache[f_hash]["manual_data"] = {}
                                self.cache[f_hash]["manual_data"]["measure"] = final_measure_str
                                self.cache[f_hash]["manual_data"]["reg2"] = final_reg2_str
                                self.save_cache()
                                
                                for res_mem in self.results:
                                    if res_mem.get('f_hash') == f_hash:
                                        res_mem["measure"] = final_measure_str
                                        res_mem["reg2"] = final_reg2_str
                                        break
                        
                        self.log(f"[*] 사용자 승인 정밀 반영 완료: {len(res_list)}건 (Row {row+1})")
            
            self.pending_reviews = [] # 검토 완료 후 비우기
