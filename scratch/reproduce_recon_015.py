...
def call_gemini(image_list, prompt, model="gemini-2.0-flash-lite"):
...
prompt = "이 이미지들 중 '2. 구성성분' 또는 '3. 구성성분' 표가 있는 페이지 번호(0부터 시작)를 찾아라. JSON응답: {\"page_index\": 숫자}"

print(f"Testing with gemini-2.0-flash-lite (default for lite in some environments)...")
res = call_gemini(recon_images, prompt, model="gemini-2.0-flash-lite-preview-02-05")
print(f"Response: {res}")

print(f"\nTesting with gemini-1.5-flash...")
res2 = call_gemini(recon_images, prompt, model="gemini-1.5-flash")
