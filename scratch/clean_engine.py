...
def extract_section3_images(pdf_path):
    """[V15.0] Section 3 영역 고해상도(2.5x) 캡처 및 폴백 원천 차단"""
    try:
        doc = fitz.open(pdf_path)
        pages = find_section3_pages(doc)
        
        # [🚨 핵심 변경] Section 3를 못 찾으면 1~3페이지 강제 캡처 폴백을 하지 않고 빈 리스트 반환
        if not pages:
            doc.close()
            return []
        
        images = []
        # 범위 내 페이지 고해상도 캡처 (2.5x 배율로 가독성 극대화)
        for p_idx in pages:
            page = doc[p_idx]
            pix = page.get_pixmap(matrix=fitz.Matrix(2.5, 2.5))
            b64_img = base64.b64encode(pix.tobytes("png")).decode("utf-8")
            images.append({"mimeType": "image/png", "data": b64_img})
            if len(images) >= 3: break # 비용 및 부하 방지 (최대 3장)
...
def call_gemini_2_5_lite(image_list=None, log_func=None, retry_instruction=None):
    """[V15.0] 순수 Vision 전용 엔진: 텍스트 입력 원천 차단"""def process_pdf(pdf_path, log_func=None):
    start_time = time.time()
    if log_func: log_func(f" 🚀 [V15.0 Vision-Only] 비전 전용 파이프라인 가동")
    ...
    hybrid_pn, _ = extract_product_name_hybrid(first_page_text, image_list, GOOGLE_API_KEY, log_func=log_func)

    # 3. 메인 AI 추출 (1차: Gemini Flash)
    final_ai_result = call_gemini_2_5_lite(image_list=image_list, log_func=log_func)
    used_engine = "Gemini-Flash"

    # 4. 2차 Fallback (GPT-4o-mini)
    if (not final_ai_result or not final_ai_result.get("구성성분")) and OPENAI_API_KEY:
        if log_func: log_func(" ├─ [Fallback] GPT-4o-mini 전환 호출 중...")
        final_ai_result = call_gpt_4o_mini(image_list=image_list, log_func=log_func)
        used_engine = "GPT-4o-mini"

    # 5. 최종 실패 판정 (신규 비즈니스 로직: 즉시 포기)
    if not final_ai_result or not final_ai_result.get("구성성분"):
        if log_func: log_func(" ❌ 모든 AI 엔진 추출 실패 (수동 검토 대상)")
        return {"error": "AI 추출 완전 실패 (수동 검토 필요)"}

    # 6. 데이터 조립 및 Phase 3 단순 후처리 (공백 제거)
    product_name = final_ai_result.get("제품명")
    if not product_name or product_name in ["미추출", "None", "none"]:
    ...
    for comp in components:
        ...
        # CAS 번호 규격 검증 (불일치 시 해당 Row 버림)
        if not re.match(r'^\d{1,7}-\d{2}-\d$', cas): 
            continue
            
        # 함유량 극단적 단순 정제 (공백 제거만)
        content = minimal_clean(comp.get("content", ""))
    ...
    return result_data
  return result_data
inal_ai_result and OPENAI_API_KEY:
    ...
    if not final_ai_result:
        ...
        for comp in components:
            ...
            if not content or any(kw in content for kw in ["-", "없음", "자료", "미기재", "비공개", "비밀", "영업비밀"]): 
                content = "미기재%"
            else:
                # [V14.6] 후처리 다이어트: 최소한의 공백 제거만 수행
                content = content.replace(" ", "")
                if "%" not in content and not any(c in content for c in "<>≤≥"): 
                    content += "%" # 기호 누락 방지 보험
