...
- A missing ending header, conflicting repeated start, ambiguous same-page or multi-page reading order, image-reading requirement, cancellation, or deadline produces `FENCE_PARTIAL` with a reason. Start, continuation, and ending pages are never admitted as the whole `page.rect`: only an observed single-column body span is allowed, exact repeated top/bottom lines are excluded except structurally repeated Section 3 table-header rows, and a line crossing a body boundary blocks the fence. Repeated structured Section 1 label/value and Section 3 CAS/concentration rows (including lightly shifted x0 values) are distinct from independent page columns; a mere wide two-column layout is not admitted. Section 1 and Section 3 are located independently.
...
- Synthetic real-PDF tests cover digital Korean/English and NFKC heading keys, exact whitespace-preserving `SectionInput` source strings, same-page and multi-page boundaries, repeated first/middle/last-page header/footer exclusion with structurally retained Korean/alternate Section 3 table headers, aligned three-column and structured two-column table admission with independent-column rejection, ToC/noise, four rotations with moved CropBox and adjacent sentinels, right-margin and header-boundary decorative-logo admission plus central/large/body-overlap/unknown-placement/same-region image blocking, bad SHA/section/rect/page/PARTIAL fence, boundary-crossing text, broken/encrypted PDFs, terminal-layout input blocking, cancellation, deadline, and no rendering.
...
| Command | Result |
| --- | --- |
| `python -m pytest tests/rebaseline -q` | 99 passed in 4.92s (pytest cache write warning only) |
| `python -m pytest tests/test_common_normalization.py -q` | 6 passed in 0.20s |
