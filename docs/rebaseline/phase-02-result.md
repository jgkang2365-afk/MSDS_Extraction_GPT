...
- A missing ending header, conflicting repeated start, ambiguous same-page or continuation-page reading order, image-reading requirement, cancellation, or deadline produces `FENCE_PARTIAL` with a reason. Continuation pages are never admitted as the whole `page.rect`: only the observed single-column body span is allowed, and exact repeated top/bottom lines are excluded. Section 1 and Section 3 are located independently.
- `PageRegion` and token coordinates are zero-based and unrotated in the CropBox coordinate space. The reader records every actual image placement, and only blocks an otherwise digital fence when that placement intersects its allowed area (or placement is unavailable). Same-region text/image is `FENCE_PARTIAL`; a confirmed `SectionInput` is always `TEXT` capability.
...
- Synthetic real-PDF tests cover digital Korean/English headings, same-page and multi-page boundaries, repeated header/footer exclusion, continuation multi-column rejection, ToC/noise, four rotations with moved CropBox and adjacent sentinels, image placement/unknown placement/same-region mixed input, bad SHA/section/rect/page/PARTIAL fence, boundary-crossing text, broken/encrypted PDFs, cancellation, deadline, and no rendering.
...
| Command | Result |
| --- | --- |
| `python -m pytest tests/rebaseline -q` | 75 passed; pytest cache write emitted one WinError 5 warning, with no test impact. |
