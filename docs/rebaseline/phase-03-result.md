...
- CAS raw substring은 Unicode dash/공백만 기존 pure normalization으로 비교값을 만들며 check digit 실패도 raw·evidence와 함께 보존한다. date와 EC number는 후보로 승격하지 않는다. 명시 `EC No.`/`EC Number`와 `CAS`가 함께 있는 표에서는 EC/CAS header의 중점 경계로 후속 행의 EC 열만 제외하며, 실제 CAS 열은 그대로 수집한다.
- 함유량 raw는 부등호, range, `Rem.`, `Balance`, `wt%`, `vol%`, `ppm` 의미를 보존한다. 같은 표에서 명시 CAS 열에 정렬된 행은 함유량 cell이 비어도 unit-header context를 유지하지만, 빈 cell 행 자체는 content candidate를 만들지 않는다. 후속 bare value는 기존과 같이 unit-bearing header column 정렬일 때만 candidate가 되며 header unit은 raw에 붙이지 않는다. 예를 들어 `CAS | [blank]` 다음 `CAS | 10` 및 `Concentration (%)`에서 raw `10`, `unit_context` `%`를 보존한다. 하나의 block에 CAS 여러 개와 content 하나가 있어도 source relation만 남기며 pairing은 하지 않는다.
...
- `python -m pytest tests/rebaseline/test_collectors.py -q`: **36 passed in 0.61s**. synthetic 및 실제 PyMuPDF PDF의 Section 3 EC/CAS/Content 별도 셀 표, 좁은 EC/CAS 열 경계, 인접 bare numeric 열 비승격, Section 1 inline 및 동일 행 label/value 뒤 multiline·company 종료 회귀를 포함한다.
- `python -m pytest tests/rebaseline -q`: **135 passed in 7.16s**.
- `python -m pytest tests/test_common_normalization.py -q`: **6 passed in 0.21s**.
- compile/import/diff check: **PASS**.
- Fresh READ_ONLY verifier: **PASS**.
