...
- 함유량 raw는 부등호, range, `Rem.`, `Balance`, `wt%`, `vol%`, `ppm` 의미를 보존한다. bare number/range는 명시 CAS header와 단위 header가 같은 표의 선행 행에 있고, 해당 CAS/단위 열의 제한된 시작 좌표 정렬에 맞을 때만 content 후보가 되며, 설명문·다른 표의 unit text와 인접 일반 숫자 열은 재사용하지 않는다. header 단위는 raw에 붙이지 않는다. 하나의 block에 CAS 여러 개와 content 하나가 있어도 source relation만 남기며 pairing은 하지 않는다.
...
- `python -m pytest tests/rebaseline/test_collectors.py -q`: **35 passed in 0.57s**. synthetic 및 실제 PyMuPDF PDF의 Section 3 EC/CAS/Content 별도 셀 표, 좁은 EC/CAS 열 경계, 인접 bare numeric 열 비승격, Section 1 inline 및 동일 행 label/value 뒤 multiline·company 종료 회귀를 포함한다.
- `python -m pytest tests/rebaseline -q`: **134 passed in 7.76s**.
- `python -m pytest tests/test_common_normalization.py -q`: **6 passed in 0.24s**.
...
- `git diff --check`: **passed**. 이번 보완의 세 파일만 변경되었고 Phase 2 보호 파일은 변경하지 않았다.
