...
def analyze_with_gemini_ensemble(v24_result, text_chunk, log_func=None, retry_instruction=None):
    ...
    3. [부등호 및 범위 통일] 함유량 기호는 아래 규칙을 절대적으로 따르세요.
       - [절대 금지] 범위 기호('~')가 쓰일 때는 부등호('≥', '≤', '<', '>')를 절대 함께 쓰지 마세요. (잘못된 예: ≥95~100% -> 올바른 예: 95~100%)
       - [일반 범위 단순화] 2개 이상의 숫자로 이루어진 범위값은 지저분한 부등호를 모두 제거하고 '~'로만 깔끔하게 연결하세요. (예: ≥70-<75% -> 70~75%)
       - [민감한 한계농도 보존] 숫자가 1개뿐인 단일 부등호(<, ≤, >, ≥)는 법적 규제 기준이므로 지우지 말고 그대로 유지하세요. (예: <1% -> <1%, ≤0.1% -> ≤0.1%, 99.9% 이상 -> ≥99.9%)
       - [특수 기호 절대 규칙] 숫자 뒤에 꼬리처럼 붙은 기호는 반드시 단일 부등호로 변환하세요. '+' 기호는 '이상(≥)'으로, '-' 기호는 '이하(≤)'로 바꿉니다. (예: 99.0 +% -> ≥99.0%, 1.0 -% -> ≤1.0%). 절대 임의로 상한선(~100%)을 만들어 범위값으로 창작하지 마세요.
       - [시약 등급(GR/EP) 통합 절대 규칙] ★필독★ 한 물질에 대해 GR(99%), EP(98%)와 같이 등급별 농도가 다르고 CAS 번호가 여러 개(수화물/무수물 등) 나열된 경우, 절대 농도를 CAS별로 나누어 1:1 매핑하지 마세요. 반드시 추출된 모든 관련 CAS 번호에 대해 동일하게 '가장 낮은(보수적인) 농도' 하나로 통일하여 적용하세요.
       - 오차범위(±): 직접 계산하여 '최소~최대%'로 변환 (예: 10 ± 2% -> 8~12%)
       - 'About', '약', 전각기호 등은 제거하고 숫자와 기호만 남기세요.
    4. [기호 없는 숫자 주의] 표의 함유량 항목에 '%' 기호 없이 단순 숫자(예: 100.0)만 적혀 있다면 퍼센트(%)로 간주하여 추출하세요. 단, CAS 번호 주변에 있는 '항 4(섹션 번호)', '분자량(18.02)', '페이지 번호' 등 문맥상 명백히 함유량이 아닌 엉뚱한 숫자를 함유량으로 착각하지 마세요. 해당 물질의 함유량 정보가 없다면 무조건 '미기재'로 처리하세요.
    5. [미기재 및 잔여량] 수치가 아예 없거나 'Residual quantity', 'Balance', '잔량' 등으로 텍스트만 적혀 있는 경우 절대 임의로 계산하지 말고 무조건 '미기재%'로 표기하세요.
    6. [혼합물 CAS 삭제] 혼합물 전체를 지칭하는 CAS 번호가 하위 성분과 중복될 경우, 혼합물 CAS는 삭제하세요.
    7. [특수 단위 배제] 함유량 단위가 '%'가 아닌 'ppm', 'mg', 'g/l' 등의 특수 단위로 표기된 경우, 절대 값을 추출하지 말고 무조건 '미기재%'로 처리하세요.

    [특별 예시 학습: 시약 등급(GR/EP)에 따른 복수 CAS 처리]
    상황: 원본에 동일 물질의 수화물/무수물 CAS 번호가 2개 있고, 함유량이 GR 99.0+%, EP 98.0+% 로 나뉘어 있는 경우
    ❌ 잘못된 출력 (1:1 매핑 금지): [{{ "cas_no": "13465-27-5", "content": "≥99.0%" }}, {{ "cas_no": "7785-87-7", "content": "≥98.0%" }}]
    🟢 올바른 출력 (최소값으로 통일): [{{ "cas_no": "13465-27-5", "content": "≥98.0%" }}, {{ "cas_no": "7785-87-7", "content": "≥98.0%" }}]
